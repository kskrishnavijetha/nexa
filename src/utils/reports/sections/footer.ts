...
export const addFooter = (doc: jsPDF): void => {
  ...
  try {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Generated on ${new Date().toLocaleDateString()}`, 190, footerPosition, { align: 'right' });
      
      // Add security logo with improved positioning
      const securityLogoPositionY = footerPosition - 5;
      doc.addImage(securityLogoDataURI, 'PNG', 87, securityLogoPositionY, 5, 5);
      
      // Add Nexabloom logo and branding with better spacing
      const logoPositionY = footerPosition;
