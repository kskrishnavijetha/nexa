...
export const generateReportPDF = async (
  ...
  return new Promise((resolve) => {
    // Use setTimeout to defer the work to a separate task
    setTimeout(async () => {
      try {
        ...
        // Memory-optimized document generation with yielding to the main thread
        ...
        if (report.industry) {
          ...
          // Limit regulations to 3 for performance
          if (report.regulations && report.regulations.length > 0) {
            const regulations = report.regulations.slice(0, 3).join(', ');
        ...
        // Process compliance issues - limit risks for better performance
        let issuesYPos = 190;
        // Check if we need a new page
        if (issuesYPos > 220) {
          doc.addPage();
        ...
        // Limit risks to 10 most critical for performance
        let limitedRisks = [];
        if (report.risks.length > 10) {
          // Prioritize high severity risks
          const highRisks = report.risks.filter(r => r.severity === 'high').slice(0, 5);
          const mediumRisks = report.risks.filter(r => r.severity === 'medium').slice(0, 3);
          const lowRisks = report.risks.filter(r => r.severity === 'low').slice(0, 2);
          limitedRisks = [...highRisks, ...mediumRisks, ...lowRisks];
        } else {
        ...
        // Process improvement suggestions - limit to 5 most important
        ...
        if (report.suggestions && report.suggestions.length > 5) {
          const limitedSuggestions = report.suggestions.slice(0, 5);
