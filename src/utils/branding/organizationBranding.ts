...
export const applyBrandingToFooter = (
  ...
  try {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Add Nexabloom logo if available from branding
      if (branding.logoUrl) {
        try {
          doc.addImage(branding.logoUrl, 'PNG', 75, logoPositionY - 5, 6, 6);
        } catch (logoError) {
          console.error("Error adding custom logo:", logoError);
          // Fallback to default Nexabloom logo
          try {
        ...
        // Use default logo if no custom logo provided
        try {
          doc.addImage(nexabloomLogo, 'PNG', 75, logoPositionY - 5, 6, 6);
        } catch (logoError) {
