
import { jsPDF } from "jspdf";

/**
 * Add footer with page numbers and Nexabloom branding to the PDF document
 * Includes integrity verification metadata and legal disclaimer
 */
export const addFooter = (doc: jsPDF, verificationMetadata?: any): void => {
  const pageCount = doc.getNumberOfPages();
  
  // Load Nexabloom logo image (using a data URI for simplicity)
  const logoDataURI = "data:image/png;base64,";
  
  // Security shield logo for integrity indicator
  const shieldLogoURI = "data:image/png;base64,";
  
  try {
    // Add header section to the first page
    doc.setPage(1);
    
    // Add "Audit Trail Report" header
    doc.setFontSize(20);
    doc.setTextColor(0, 51, 102); // Dark blue color
    doc.text('Audit Trail Report', 20, 20);
    
    // Add document details section
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0); // Black text
    
    const docName = verificationMetadata?.documentName || "Document";
    const today = new Date();
    const formattedDate = `${today.getMonth() + 1}/${today.getDate()}/${today.getFullYear()}`;
    
    doc.text(`Document: ${docName}`, 20, 35);
    doc.text(`Export Date: ${formattedDate}`, 20, 45);
    
    // Add Executive Summary header 
    doc.setFontSize(16);
    doc.setTextColor(0, 51, 102); // Dark blue color for heading
    doc.text('Executive Summary', 20, 60);
    
    // Add Integrity Verification section
    doc.setFontSize(14);
    doc.setTextColor(0, 128, 0); // Green color for integrity verification
    doc.text('Integrity Verification', 20, 80);
    
    // Add Organization and Industry details
    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0); // Black text
    
    const orgName = "XYZ Energy Compliance and Infrastructure Security Policy.pdf"; 
    const industry = "Energy & Utilities";
    
    // Add hash details
    if (verificationMetadata && verificationMetadata.hash) {
      doc.text(`Organization: ${orgName}`, 20, 95);
      doc.text(`Hash: ${verificationMetadata.hash.substring(0, 32)}...`, 20, 105);
      doc.text(`Industry: ${industry}`, 20, 115);
      doc.text(`Timestamp: ${verificationMetadata.timestamp}`, 20, 125);
      doc.text(`Regulations: GDPR, CCPA`, 20, 135);
      doc.text(`Generated By: AI Compliance System`, 20, 145);
    }
    
    // Add a horizontal line to separate the header from content
    doc.setDrawColor(200, 200, 200);
    doc.setLineWidth(0.5);
    doc.line(20, 160, 190, 160);
    
    // Add footer to each page
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i);
      
      // Add a horizontal line at the top of the footer
      doc.setDrawColor(200, 200, 200);
      doc.setLineWidth(0.5);
      doc.line(20, 265, 190, 265);
      
      // Add legal disclaimer centered
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      const disclaimer = "LEGAL DISCLAIMER: This report is for informational purposes only and does not constitute legal advice. Integrity protected by SHA-256 verification.";
      doc.text(disclaimer, 105, 280, { align: 'center' });
      
      // Add Nexabloom branding (centered)
      doc.setTextColor(79, 70, 229); // Indigo color for branding
      doc.text('Nexabloom', 105, 276, { align: 'center' });
      
      // Add Nexabloom URL (right aligned)
      doc.text('nexabloom.xyz', 190, 276, { align: 'right' });
      
      // Add shield logo on the left
      doc.addImage(shieldLogoURI, 'PNG', 20, 272, 6, 6);
      
      // Add page number
      doc.setTextColor(100, 100, 100);
      doc.text(`Page ${i} of ${pageCount}`, 30, 276);
    }
  } catch (error) {
    console.error("Error adding footer to PDF:", error);
    
    // Fallback to simpler text-only footer if image loading fails
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i);
      
      doc.setDrawColor(200, 200, 200);
      doc.setLineWidth(0.5);
      doc.line(20, 265, 190, 265);
      
      // Add legal disclaimer
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      const disclaimer = "LEGAL DISCLAIMER: This report is for informational purposes only and does not constitute legal advice. Integrity protected by SHA-256 verification.";
      doc.text(disclaimer, 105, 280, { align: 'center' });
      
      // Add Nexabloom branding
      doc.setTextColor(79, 70, 229);
      doc.text('Nexabloom', 105, 276, { align: 'center' });
      doc.text('nexabloom.xyz', 190, 276, { align: 'right' });
      
      // Add page number text
      doc.setTextColor(100, 100, 100);
      doc.text(`Page ${i} of ${pageCount}`, 30, 276);
    }
  }
};
