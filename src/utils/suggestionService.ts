...
export function generateSuggestions(regulations: string[] = [], region?: Region): string[] {
  ...
  if (regulations.includes('GDPR') || regulations.length === 0) {
    suggestions.push('Review and update privacy policies to comply with GDPR.');
  }
  
  if (regulations.includes('HIPAA') || regulations.length === 0) {
    suggestions.push('Implement stronger access controls to protect PHI.');
  }
  
  if (regulations.includes('SOC 2') || regulations.length === 0) {
    suggestions.push('Enhance security monitoring and incident response.');
  }
  
  // Add region-specific suggestions
  if (region && REGION_SUGGESTIONS[region]) {
    suggestions = [...suggestions, ...REGION_SUGGESTIONS[region]];
  }
