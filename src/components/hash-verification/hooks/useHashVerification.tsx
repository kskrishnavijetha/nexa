...
export const useHashVerification = () => {
  ...
  const handleFileSelection = async (selectedFile: File) => {
    if (!canUseHashVerification) {
      toast.error('Hash verification is only available in Pro and Enterprise plans.');
      return;
    }
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      if (file && user) {
        await logVerificationEvent(user.id, file.name, isMatch);
      }
    } catch (error) {
  ...
  return {
    file, computedHash, compareMethod, pastedHash, setPastedHash,
    selectedDocumentId, setSelectedDocumentId, recentDocuments,
    isVerifying, verificationResult, isLoading, setCompareMethod,
    handleFileSelection, handleVerification, canUseHashVerification
  };
};
